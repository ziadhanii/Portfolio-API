    ...
    : BaseApiController
{
    ...
    public async Task<IActionResult> GetGitHubProfile()
    {
        try
        {
            var profile = await gitHubService.GetUserAsync();
            return Ok(profile);
        }
        catch (HttpRequestException ex)
        {
            logger.LogError(ex, "Error fetching GitHub profile");
    ...
    public async Task<IActionResult> GetFeaturedProjects()
    {
        try
        {
            var projects = await gitHubService.GetFeaturedProjectsAsync();
            return Ok(projects);
        }
        catch (HttpRequestException ex)
        {
            logger.LogError(ex, "Error fetching featured projects");
    ...
    public async Task<IActionResult> GetPrivateRepos()
    {
        try
        {
            var repos = await gitHubService.GetPrivateReposAsync();
            return Ok(repos);
        }
        catch (HttpRequestException ex)
        {
            logger.LogError(ex, "Error fetching private repositories");
    ...
    public async Task<IActionResult> GetGitHubRepos([FromQuery] int page = 1, [FromQuery] int perPage = 30)
    {
        try
        {
            var repos = await gitHubService.GetReposAsync(page, perPage);
            return Ok(repos);
        }
        catch (HttpRequestException ex)
        {
            logger.LogError(ex, "Error fetching GitHub repositories");
